# Metadata Overview - MII IG Kerndatensatz-Modul Molekulargenetischer Befundbericht v2026.0.4
...
> **Optional page (0..1).** The KDS module menu lists this page as **optional** — keep it only when your module's profiles carry the metadata characteristics it documents (as, for example, the Base module does). Decide for your module: **keep** it — fill it in and delete this banner and the `OPTIONAL-PAGE` marker comment (in this file AND the German mirror) — or **remove** it, following the per-entry procedure in [`docs/optional-pages.md`](https://github.com/medizininformatik-initiative/kerndatensatzmodul-GenetischeTests/blob/main/docs/optional-pages.md) of this repository. A release must not ship with this banner (convention check M9).

### Metadata Overview

This page documents the computable metadata of the **Molekulargenetischer Befundbericht** module. The metadata exists so that the artifacts of this guide can be discovered, evaluated, validated, cited, governed and reused — by people and by software alike.
...
The following CRMI-related metadata is set in [`sushi-config.yaml`](https://github.com/medizininformatik-initiative/kerndatensatzmodul-GenetischeTests/blob/main/sushi-config.yaml) and ends up in the generated `ImplementationGuide` resource.
...
| [Resource Approval Date](http://hl7.org/fhir/extensions/5.3.0/StructureDefinition-resource-approvalDate.html) | Artifact lifecycle; publishing; governance | `ImplementationGuide.extension`(`2026-01-02`) | Records the date on which the publisher officially approved the content for use. |
...
| | | | |
| :--- | :--- | :--- | :--- |
| F1 | RDA-F1-01M | Metadata is identified by a persistent identifier | Canonical`url`values, the package id`de.medizininformatikinitiative.kerndatensatz.molgen`, the package version and`package-source`. Persistence depends on publication governance. |
...
| R1.1 | RDA-R1.1-01M | Metadata includes licence information | TODO:REVIEW — this module asserts NO guide-level`license:`(the source declares none; see the note on`license:`in sushi-config.yaml and the copyright section of the home page). Until Gate A decides one, the only licence information is the home page copyright notice ("the content of this specification is public; reuse and republication claims are not restricted"). Reuse conditions for production clinical data must come from the data providers. |
