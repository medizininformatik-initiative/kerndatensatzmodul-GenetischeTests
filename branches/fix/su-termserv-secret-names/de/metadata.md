# Metadaten-Übersicht - MII IG Kerndatensatz-Modul Molekulargenetischer Befundbericht v2026.0.4
...
> **Optionale Seite (0..1).** Das KDS-Modulmenü führt diese Seite als **optional** — behalten Sie sie nur, wenn die Profile Ihres Moduls die hier dokumentierten Metadaten-Charakteristiken aufweisen (wie z. B. das Basis-Modul). Entscheiden Sie für Ihr Modul: Seite **behalten** — Inhalte ausfüllen und dieses Banner samt `OPTIONAL-PAGE`-Marker-Kommentar löschen (in dieser Datei UND in der englischen Quellseite) — oder Seite **entfernen**, nach der Schritt-für-Schritt-Anleitung in [`docs/optional-pages.md`](https://github.com/medizininformatik-initiative/kerndatensatzmodul-GenetischeTests/blob/main/docs/optional-pages.md) dieses Repositories. Ein Release darf dieses Banner nicht enthalten (Konventions-Check M9).

### Metadaten-Übersicht

Diese Seite beschreibt die maschinenlesbaren Metadaten des Moduls **Molekulargenetischer Befundbericht**. Sie existieren, damit die Artefakte dieses Leitfadens auffindbar, bewertbar, validierbar, zitierbar, steuerbar und wiederverwendbar sind — für Menschen wie für Software.
...
Die folgenden CRMI-Metadaten werden in [`sushi-config.yaml`](https://github.com/medizininformatik-initiative/kerndatensatzmodul-GenetischeTests/blob/main/sushi-config.yaml) gesetzt und landen in der erzeugten `ImplementationGuide`-Ressource.
...
| [Resource Approval Date](http://hl7.org/fhir/extensions/5.3.0/StructureDefinition-resource-approvalDate.html) | Lebenszyklus; Veröffentlichung; Governance | `ImplementationGuide.extension`(`2026-01-02`) | Hält fest, wann der Herausgeber den Inhalt offiziell zur Nutzung freigegeben hat. |
...
| | | | |
| :--- | :--- | :--- | :--- |
| F1 | RDA-F1-01M | Metadaten sind über einen persistenten Identifikator identifiziert | Kanonische`url`-Werte, die Paket-Id`de.medizininformatikinitiative.kerndatensatz.molgen`, die Paketversion und`package-source`. Die Persistenz hängt von der Veröffentlichungs-Governance ab. |
...
| R1.1 | RDA-R1.1-01M | Metadaten enthalten Lizenzinformationen | TODO:REVIEW — dieses Modul deklariert KEINE`license:`auf Leitfaden-Ebene (die Quelle deklariert keine; siehe den Hinweis bei`license:`in sushi-config.yaml und den Copyright-Abschnitt der Startseite). Bis Gate A eine Lizenz entscheidet, ist die einzige Lizenzinformation der Copyright-Hinweis der Startseite („Der Inhalt dieser Spezifikation ist öffentlich; die Nachnutzungs- bzw. Veröffentlichungsansprüche sind nicht beschränkt"). Nutzungsbedingungen produktiver klinischer Daten kommen von den Datenhaltenden. |
