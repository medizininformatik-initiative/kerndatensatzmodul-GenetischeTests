# Artefaktübersicht - MII IG Kerndatensatz-Modul Molekulargenetischer Befundbericht v2027.0.0-ballot.rc1
...
| | |
| :--- | :--- |
| [ mii-sp-molgen-diagnostic-report-genomic-study  ](SearchParameter-mii-sp-molgen-diagnostic-report-genomic-study.md) | Finds the reports produced by a given genomic study, which is the link from the clinical report back to the investigation behind it. The reference sits in the genomic-study-reference extension; DiagnosticReport has no base search parameter for it. |
| [ mii-sp-molgen-diagnostic-report-recommended-action  ](SearchParameter-mii-sp-molgen-diagnostic-report-recommended-action.md) | Finds the reports that recommend a given follow-up action, such as a confirmatory test or a therapy recommendation. The reference sits in the recommended-action extension and is not reachable through DiagnosticReport.result. |
| [ mii-sp-molgen-genomic-study-analysis-device  ](SearchParameter-mii-sp-molgen-genomic-study-analysis-device.md) | Finds analyses run on a given sequencer or analytical device, for tracing results back to the instrument that produced them. The device sits in the genomic-study-analysis-device extension, not in Procedure. |
| [ mii-sp-molgen-genomic-study-analysis-method  ](SearchParameter-mii-sp-molgen-genomic-study-analysis-method.md) | Finds analyses by the sequencing or genotyping method applied (for example targeted panel, whole exome, whole genome). The method sits in the genomic-study-analysis-method-type extension, which no base Procedure search parameter reaches. |
| [ mii-sp-molgen-genomic-study-analysis-regions-studied  ](SearchParameter-mii-sp-molgen-genomic-study-analysis-regions-studied.md) | Finds analyses that actually examined a given gene or genomic region. This is what distinguishes a variant that was not found from a region that was never examined: without it a query cannot tell the two apart. The value sits in the studied sub-extension of genomic-study-analysis-regions. |
| [ mii-sp-molgen-genomic-study-analysis-specimen  ](SearchParameter-mii-sp-molgen-genomic-study-analysis-specimen.md) | Finds analyses performed on a given specimen. The specimen sits in the genomic-study-analysis-specimen extension, so the base Procedure.specimen search parameter does not reach it. |
